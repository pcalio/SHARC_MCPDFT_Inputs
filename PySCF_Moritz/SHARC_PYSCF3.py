...
import numpy
#import math
from mrh.my_pyscf.tools.molcas2pyscf import get_mo_from_h5, get_mol_from_h5
...
#from pyscf import lib,tools,gto,scf,mcscf
...
version='3.0'
versiondate=datetime.date(2021,10,18)
...
# hash table for conversion of functional names from OpenMolcas to PySCF
FUNCTIONAL={
                'FTLSDA': 'ftLDA,VWN3',
                'TPBE': 'tPBE'
                }
...
def printheader():
    ...
    string+='||'+' '*19+'Modified by Paul Calio and Moritz Baumgarten'+' '*17+'||\n'
    string+='||'+' '*80+'||\n'
    string+='||'+' '*(36-int((len(version)+1)/2))+'Version: %s' % (version)+' '*(35-int((len(version))/2))+'||\n'
    lens=len(versiondate.strftime("%d.%m.%y"))
    string+='||'+' '*(37-int(lens/2))+'Date: %s' % (versiondate.strftime("%d.%m.%y"))+' '*(37-int((lens+1)/2))+'||\n'
    string+='||'+' '*80+'||\n'
    string+='  '+'='*80+'\n\n'
    print(string)
    if DEBUG:
...
def printcomplexmatrix(matrix,states):
    ...
    string='Real Part:\n'
    string+='-'*(11*nmstates+int(nmstates/3))
    string+='\n'
    istate=0
...
def printQMout(QMin,QMout):
    ...
    if 'nacdr' in QMin:
        ...
        for imult,i,msi in itnmstates(states):
            ...
            istate+=1
    # Overlaps
    if 'overlap' in QMin and not QMin['template']['pure-pyscf']: #FLAG
...
# ======================================================================= #
def getcienergy(QMin,out,mult,state,version,method,dkh):
    ...
    if True:
        if method==0:
            ...
            stateindex=3
            enindex=6
        elif method==3 and not QMin['template']['pyscf']:
            modulestring='&MCPDFT'
            spinstring='Spin quantum number'
...
#TODO
        ...
        for i, line in enumerate(out):
            if modulestring in line:
                ...
                if int(2*spin)+1==mult:
                    correct_mult = True
                    f.write('\nspin='+str(spin)+' and mult='+str(int(2*spin)+1)+'   -> correct mult found')
            elif energystring in line and module and correct_mult:
                if method==0 and dkh:
                    l=out[i+4+state].split()
                    f.write('return line: '+line)
                    return float(l[1])
                else:
                    l=line.split()
                    if int(l[stateindex])==state:
                        return float(l[enindex])
    f.write('module'+str(module)+' correct_mult'+str(correct_mult))
    f.close()
...
def getcidm(out,mult,state1,state2,pol,version):
    ...
    for jline,line in enumerate(out[iline+1:]):
        ...
        if statesstring in line:
            ...
            if len(jobiphmult)==2:
                stateshift=int(nstates/2)
            else:
                stateshift=0
        if matrixstring in line:
            block=int((stateshift+state2-1)/4)
...
def getsocme(QMin,out, mult1, state1, ms1, mult2, state2, ms2, states, version, method, dkh):
    ...
    if mult1==mult2 and state1==state2 and ms1==ms2:
        return complex(getcienergy(QMin,out,mult1,state1,version,method,dkh), 0.0)
...
def getsmate(out,mult,state1,state2,states):
    ...
    for jline,line in enumerate(out[iline+1:]):
        ...
        if matrixstring in line:
            rowshift=1
            for i in range(int(nstates/2+state2-1)):
                rowshift+=int(i/5+1)
            rowshift+=int(1+(state1-1)/5)
            colshift=(state1-1)%5

            return float(out[iline+jline+rowshift+1].split()[colshift])

# ======================================================================= #
def getQMout(out,QMin,gradmaps,job):
    ...
    if QMin['template']['pyscf'] or QMin['template']['pure-pyscf']:   
        #mults ={}
        #mol_objs = {}

        if QMin['template']['pure-pyscf']:
            for mode in ['soc']: #phases,nacdr, overlap
                if mode in QMin:
                    print('%s currently not implemented in this interface for pure PySCF calculation :(' %mode)
                    sys.exit(123459)

        if 'h' in QMin:
            hpy = makecmatrix(nmstates,nmstates)
            diags = []
            mults_h = []
            mol_objs_h = []

            for istate,i in enumerate(QMin['statemap'].values()):   #optimize this loop
                mult,state,ms=tuple(i)
                if mult in mults_h:
                    hpy[istate][istate] = diags[mults_h.index(mult)][state-1]
                    continue
                ss = (QMin['template']['roots'] == 1)

                mol_obj = get_energy(QMin,mult,state-1,gssorb = get_gssfile(QMin,job))
                if ss:
                    hpy[istate][istate] = complex(mol_obj.e_tot)
                    diags.append([complex(mol_obj.e_tot)])
                else:
                    if QMin['method'] == 4:
                        hpy[istate][istate] = complex(mol_obj._e_states[state-1])
                        diags.append([complex(num) for num in mol_obj._e_states])
                    else:    
                        hpy[istate][istate] = complex(mol_obj.e_states[state-1])
                        diags.append([complex(num) for num in mol_obj.e_states])
                mol_objs_h.append(mol_obj)
                mults_h.append(mult)
            QMout['h']=hpy
            ...
            for istate,i in enumerate(QMin['statemap'].values()):
                for jstate,j in enumerate(QMin['statemap'].values()):
                    ...
                    if states[mult1-1]==1 and method==2:
                        ...
                        method1=method
                    if mult1 == mult2 and state1 == state2 and ms1 == ms2:
                        if mult1 in mults_soc:
                            hpy[istate][jstate] = diags[mults_soc.index(mult1)][state1-1]
                            continue
                        ss = (QMin['template']['roots'][mult1-1] == 1)
                        mol_obj= get_energy(QMin,mult1,state1-1,gssorb = get_gssfile(QMin,job))
                        if ss:
                            hpy[istate][jstate] = complex(mol_obj.e_tot)
                            diags.append([complex(mol_obj.e_tot)])
                        else:
                            if QMin['method'] == 4:
                                hpy[istate][jstate] = complex(mol_obj._e_states[state1-1])
                                diags.append([complex(num) for num in mol_obj._e_states])
                            else:
                                hpy[istate][jstate] = complex(mol_obj.e_states[state1-1])
                                diags.append([complex(num) for num in mol_obj.e_states])
                        mol_objs_soc.append(mol_obj)
                        mults_soc.append(mult1)
                    else:
                        hpy[istate][jstate] = getsocme(QMin, out, mult1, state1, ms1, mult2, state2, ms2 ,QMin['states'],version,method1,not QMin['template']['no-douglas-kroll'])
            QMout['h']=hpy
        # Grad:  returns a list of nmstates vectors
        if 'grad' in QMin:
            if QMin['gradmap'] in gradmaps:
                QMout['grad'] = gradmaps[gradmaps.index(QMin['gradmap'])+1]
            else:
                mults_grad = []
                gradpy = []
                mol_objs_grad = []
                ...
                for istate,i in enumerate(QMin['statemap'].values()):
                    ...
                    if (mult,state) in QMin['gradmap']:
                        if (mult,state) in calculated:
                            gradpy.append(grads_tmp[calculated.index((mult,state))])
                            continue
                        elif mult in mults_grad:
                            mol_obj = mol_objs_grad[mults_grad.index(mult)]
                        else:
                            if 'h' in QMin: #fancy statemap stuff here
                                mol_obj = mol_obj_h[istate]
                            elif 'soc' in QMin: #fancy statemap stuff here
                                mol_obj = mol_obj_soc[istate]
                            else:
                                mol_obj = get_energy(QMin,mult,state-1,gssorb = get_gssfile(QMin,job))
                                mol_objs_grad.append(mol_obj)
                                mults_grad.append(mult)    #adds new multiplicity -> we want to add a pool here for all gradients of same multiplicity for the states in same multiplicity

                        gradpy_array = list(get_grad(QMin,mol_obj,state-1))
                        gradpy_list = [list(array) for array in gradpy_array]
                        gradpy.append(gradpy_list)
                        grads_tmp.append(gradpy_list)
                        calculated.append((mult,state))
                    else:
                        gradatom=[]
                        for iatom in range(natom):
                            gradatom.append([0.,0.,0.])
                        gradpy.append(gradatom)
                QMout['grad'] = gradpy
        ...
        # DM: get vector of three dipole matrices, three nested loops, returns a list of three matrices(nmstates,nmstates)
         ...
         #   mults_dm = []
          #  mol_objs_dm = []
           # calculated_dm=[]
            #states_dm = []
            #dm = [makecmatrix(nmstates,nmstates) for _ in range(3)]
              ...
              #      mult1,state1,ms1=tuple(QMin['statemap'][i])
               #     mult2,state2,ms2=tuple(QMin['statemap'][j])
                #    if mult1==mult2 and ms1==ms2:
                 #       if (state1,state2) in states_dm:
                  #          for xyz in range(3):
                   #             dm[xyz][istate][jstate]=calculated_dm[states_dm.index((state1,state2,mult1,ms1))][xyz]
                    #        continue
                     #   elif (state2,state1) in states_dm:
                      #      for xyz in range(3):
                       #         dm[xyz][istate][jstate]=calculated_dm[states_dm.index((state2,state1,mult1,ms1))][xyz]
                        #    continue
                        #check wheter we have a mc with mult1 already stored, if not claculate it
                        #if 'grad' in QMin and mult1 in mults_grad:
                         #   mc = mol_objs_grad[mults_grad.index(mult1)]
                        #elif 'soc' in QMin and mult1 in mults_soc:
                         #   mc = mol_objs_soc[mults_soc.index(mult1)]
                            
                        #elif 'h' in QMin and mult1 in mults_h:
                         #   mc = mol_objs_h[mults_h.index(mult1)]
                            
                        #else:
                         #   mc = get_energy(QMin,mult1,state1-1,gssorb = get_gssfile(QMin,job))
                          #  mults_dm.append(mult1)
                           # mol_objs_dm.append(mc)
                            
                        #assert(mc.fcisolver.smult == mult1)
                        #dm_vec = get_dm(QMin,mc,state1-1,state2-1)
                        #for xyz in range(3):
                         #   dm[xyz][istate][jstate] = complex(dm_vec[xyz])


                        #calculated_dm.append(dm_vec)
                        #states_dm.append((state1,state2,mult1,ms1))
                    #else:
                     #   print('istate: ',istate, '\njstate: ',jstate,'\n\ndm: ',dm)
                      #  for xyz in range(3):
        ...
        # no spin-orbit couplings, hamilton operator diagonal, only one loop
            ...
            for istate,i in enumerate(QMin['statemap'].values()):
                ...
                if states[mult-1]==1 and method==2:
                    ...
                    method1=method
                h[istate][istate]=complex(getcienergy(QMin,out,mult,state,version,method1,not QMin['template']['no-douglas-kroll']))
            QMout['h']=h
            ...
            for istate,i in enumerate(QMin['statemap'].values()):
                for jstate,j in enumerate(QMin['statemap'].values()):
                    ...
                    if states[mult1-1]==1 and method==2:
                        ...
                        method1=method
                    soc[istate][jstate]=getsocme(QMin, out, mult1, state1, ms1, mult2, state2, ms2 ,QMin['states'],version,method1,not QMin['template']['no-douglas-kroll'])
            QMout['h']=soc
        # Grad:  returns a list of nmstates vectors        
        if 'grad' in QMin:
            grad=[]
...
def readQMin(QMinfilename):
    ...
    booleans=['cholesky','no-douglas-kroll','qmmm','cholesky_analytical','pyscf','no-molcas-init','pure-pyscf']
    for i in booleans:
    ...
    for line in template:
        ...
        #elif 'pyscf' in line[0]:
        #    QMin['template']['pyscf'] = True
        #elif 'no-molcas-init' in line[0]:
        #    QMin['no-molcas-init'] = True
    ...
    if QMin['template']['no-molcas-init'] and not QMin['template']['pyscf']:
        print('Trying to use MOLCAS without MOLCAS guesses? Hm')
        sys.exit(1234)

    if QMin['template']['pure-pyscf']:
        QMin['template']['no-molcas-init'] = True
    ...
    # find method
    allowed_methods=['casscf','caspt2','ms-caspt2','mcpdft','cmspdft'] #MCPDFT: QMin['method'] == 3, CASSCF: QMin['method'] == 0 & CMS-PDFT: QMin['method'] == 4 
    ...
    #check CMS applicability
    for root in QMin['template']['roots']:
        if root == 1 and QMin['method'] == 4:
            print('State Specific CMS is not possible!')
            sys.exit(12349)
    ...
    #if QMin['method'] == 4 and not QMin['template']['pyscf']:
     #   print('OpenMolcas only supports CMS-PDFT energies right now. This interface is not capapble of doing CMS-PDFT dynamics in OpenMolcas. Add the keywort "pyscf" in the template to do dynamics with CMS-PDFT in PySCF.')
    #    sys.exit(12350)

    #if QMin['template']['no-molcas-init']:
    #    if QMin['method']==0:
    #        QMin['gssfile']=os.path
...
def gettasks(QMin):
    ...
    for imult,nstates in list_to_do:
        ...
        if not 'samestep' in QMin or 'always_orb_init' in QMin:
            ...
            if QMin['template']['pyscf']:
                tasks.append(['copy','MOLCAS.rasscf.h5','MOLCAS.%i.h5' %(imult+1)])
            if QMin['method'] in [3,4]:
                tasks.append(['copy','MOLCAS.JobIph','MOLCAS.%i.JobIph' % (imult+1)])
                tasks.append(['copy','MOLCAS.JobIph','MOLCAS.JobOld'])
                if not QMin['template']['pyscf']:                    
                ...
                if i[0]==imult+1:
                    if QMin['template']['roots'][imult]==1:
                        ...
                        if 'samestep' in QMin:
                            ...
                            if QMin['method'] in [3,4] and not QMin['template']['pyscf']:
                                tasks.append(['copy','MOLCAS.JobIph','MOLCAS.JobOld'])
                                fxnal=QMin['functional']
                        ...
                        tasks.append(['link',os.path.join(QMin['scratchdir'],'master','MOLCAS.NqGrid'),'MOLCAS.NqGrid'])
...
def writeMOLCASinput(tasks, QMin):
    ...
    for task in tasks:
        ...
        elif task[0]=='rasscf-rlx' and QMin['method'] in [3,4]:
            nactel=QMin['template']['nactel']
            if (nactel-task[1])%2==0:
                nactel-=1
            string+='&RASSCF\nSPIN=%i\nNACTEL=%i 0 0\nINACTIVE=%i\nRAS2=%i\nCIROOT=%i %i 1\nRLXROOT=%i\n' % (
...
def runjobs(joblist,QMin):
    ...
    for ijobset,jobset in enumerate(joblist):
        ...
        if 'master' in jobset:
            ...
            if not QMin['template']['pure-pyscf']:
                saveJobIphs(WORKDIR,jobset['master'])

        print('')

    for i in errorcodes:
        if not QMin['template']['pure-pyscf'] or not QMin['template']['no-molcas-init']:
...
def collectOutputs(joblist,QMin,errorcodes):
    ...
    #    for job in jobset:
    #        if errorcodes[job] == 0:
    #            if jobset[job]['gradmap'] not in gradmaps:
    #                gradmaps.append(jobset[job]['gradmap'])
    #            for key,value in jobset[job]['statemap'].values()
                    

    for ijobset,jobset in enumerate(joblist):
        pool = Pool(processes = QMin['nslots_pool'][ijobset])
        for job in jobset:
            if errorcodes[job]==0 and not QMin['template']['pure-pyscf']:
                outfile=os.path.join(QMin['scratchdir'],job,'MOLCAS.out')
                print('Reading %s' % (outfile))
                out=readfile(outfile)
                
                #QMout[job]=getQMout(out,jobset[job],gradmaps = gradmaps,job=job)
                QMout[job] = pool.apply_async(getQMout, [out,jobset[job],gradmaps,job])

                #if jobset[job]['gradmap'] not in gradmaps:
                #    gradmaps.append(jobset[job]['gradmap'])
                #    gradmaps.append(QMout[job]['grad'])

                if 'displacement' in jobset[job]:
                    QMout[job]=verifyQMout(QMout[job],jobset[job],out)
            #elif errorcodes == 'pure-pyscf':
            elif QMin['template']['pure-pyscf']:
                out = ''
                QMout[job] = pool.apply_async(getQMout, [out,jobset[job],gradmaps,job])

            else:
                if ('master' in job or 'grad' in job) and not QMin['template']['pure-pyscf']:
...
def moveJobIphs(QMin):
    ...
    # deletes also all old .master filesi
...
def stripWORKDIR(WORKDIR):
    ...
    keep=['MOLCAS.out','MOLCAS\.[1-9]\.JobIph','MOLCAS\.[1-9]\.RasOrb','MOLCAS\.[1-9]\.molden','MOLCAS.rasscf.h5','MOLCAS.gssorb']
    for ifile in ls:
        delete=True
        for k in keep:
...
#get MOLCAS initial guess with GUESSORB


def get_mcpdft_energy(QMin,mult,state,x2c=False,gssorb=None):
    '''Calculates MC-PDFT energy.'''
    #gssorb is path to MOLCAS.guessorb.h5 or MOLCAS.rasscf.h5

    geom = QMin['geo']
    basis = BASIS[QMin['template']['basis'].upper()]
    functional = FUNCTIONAL[QMin['template']['functional'].upper()]
    nactele = QMin['template']['nactel']
    ncas = QMin['template']['ras2']
    if DEBUG:
        output = 'pyscf_%i_%i_%i.log' %(mult,state,random.randint(0,99999))
    else:
        output=None
    roots = QMin['template']['roots']
    ss = roots[mult-1]==1
    if not ss:
        cas_weights = get_sa_weights(roots[mult-1])

    #if stateaverage:
    #npad=QMin['template']['rootpad'][task[1]-1] -> for multiplicity task[1]-1 -> if npad == 0: cas_weights = get_sa_weights(QMin['template']['roots'][mult-1]) that state averages over one multiplicity
         
    #moldenfile = './MOLCAS.guessorb.molden'

    if not gssorb:
        mol = gto.M(atom = geom, basis = basis, symmetry = False, verbose = logger.INFO, output = output)
        if x2c:
            mf = scf.RHF(mol).x2c().run()
        else:
            mf = scf.RHF(mol).run()
        mc = mcpdft.CASSCF(mf, functional, ncas, nactele, grids_attr = quasi_ultrafine)
        mc.fcisolver = csf_solver(mol, smult = mult)
        if not ss:
            mc.state_average_(cas_weights)
        else:
            mc.state_specific_(state = state)   #fix this , but in general state_specific_ has bad convergence for state > 1
        e_tot, e_ot, e_mcscf, e_cas, ci, mo_coeff, mo_energy = list(mc.kernel())

    else:
        mol = get_mol_from_h5(gssorb)
        mol.output = output
        mol.verbose = logger.INFO
        mol.symmetry = False
        mol.build()
        mo_coeff = get_mo_from_h5(mol,gssorb)
        ...
        if not ss:
            ...
            mc.state_specific_(state = state)
        mc.mo_coeff = mo_coeff
        e_tot,e_ot,e_mcscf,e_cas,ci,mo_coeff,mo_energy = list(mc.kernel(mo = mo_coeff)) 
        
    return mc,e_tot, e_ot, e_mcscf, e_cas, ci, mo_coeff, mo_energy


def get_mcpdft_grad(mc_final,state):
    mc_grad = mc_final.nuc_grad_method()
    dE = mc_grad.kernel(state=state)
    return dE

def get_mcpdft_dm(mc,state1,state2):   #is not able to return dipole off-diagonals!!!!
    
    if state1 == state2:
        dm = mc.dip_moment(state=state1,unit='Debye')
    else:
        dm = get_casscf_dm(mc,state1,state2)
    
    return dm

def get_cms_energy(QMin,mc,mult,weights):
    assert(mult==mc.fcisolver.smult)
    roots = QMin['template']['roots']
    ss = roots[mult-1]==1
    if not ss:
        cas_weights = get_sa_weights(roots[mult-1])
    else:
        print('SS is not yet implemeted here for CMS-PDFT   -> add extra input in MOLCAS.template for number of CMS states and use it for weights')
        sys.exit(123459)
...
def get_cms_grad(mc,state):
    mc_grad = sipdft_grad.Gradients(mc)
    dE = mc_grad.kernel(state=state)
    return dE

def get_sa_weights(n):
    if n == 1:
        return 1.0
    else:
        return [1.0/n for _ in range(n)]

def get_casscf_energy(QMin,mult,state,x2c=False,gssorb=None):
    ...
    if not gssorb:
        ...
        if x2c:
            ...
            mf = scf.RHF(mol).run()
        mc = mcscf.CASSCF(mf,ncas,nactele) 
        mc.fcisolver = csf_solver(mol, smult = mult)
        if not ss:
            mc.state_average_(cas_weights)
        else:
            mc.state_specific_(state = state)
        e_tot, e_cas, ci, mo_coeff, mo_energy = list(mc.kernel())
        ...
        e_tot, e_cas, ci, mo_coeff, mo_energy = list(mc.kernel(mo_coeff = mo_coeff))
    
    return mc, e_tot, e_cas, ci, mo_coeff, mo_energy

def get_casscf_grad(mc,state):
    dE = mc.Gradients().kernel(state=state)
    return dE

def get_casscf_dm(mc,state1,state2):
    
    #add state-specific stuff here: -> no off-diags and only dipole moments needed
    ...
    #overlap_ints = mc.mol.intor('cint1e_ovlp_sph',comp=1) -> needed when AO basis non orthogonal (not the cas with PySCF)

    t_dm1 = mc.fcisolver.trans_rdm1(mc.ci[state1], mc.ci[state2], mc.ncas, mc.nelecas)
    
    orbcas = mc.mo_coeff[:,mc.ncore:mc.ncore+mc.ncas]
    t_dm1_ao = reduce(numpy.dot, (orbcas, t_dm1, orbcas.T))

    if state1 == state2: 
        core_orb = mc.mo_coeff[:,0:mc.ncore]
        t_dm1_core = reduce(numpy.dot, (core_orb, numpy.diag(numpy.array([2]*core_orb.shape[1])), core_orb.T))

        dm = -1*(numpy.einsum('xij,ji->x', dip_ints, t_dm1_ao).real + numpy.einsum('xij,ji->x', dip_ints, t_dm1_core).real) + charge_center

    else:
        dm = -1*numpy.einsum('xij,ji->x', dip_ints, t_dm1_ao).real

    return [i for i in dm]


def get_energy(QMin,mult,state,gssorb=None):   #cut x3c and gssorb param-> determinable with QMin
    
    x2c = not QMin['template']['no-douglas-kroll']

    if QMin['method'] == 0:
        mc,_,_,_,_,_ = get_casscf_energy(QMin,mult,state,x2c,gssorb)
    elif QMin['method'] == 3:
        mc,_,_,_,_,_,_,_ = get_mcpdft_energy(QMin,mult,state,x2c,gssorb)
    elif QMin['method'] == 4:
        mc,_,_,_,_,_,_,_ = get_mcpdft_energy(QMin,mult,state,x2c,gssorb)
        weights = get_sa_weights(len(mc.ci))
        mc = get_cms_energy(QMin,mc,mult,weights)
    else:
        print('Energies: No PySCF support for given method!')
        sys.exit(12345)

    return mc


def get_grad(QMin,mc,state):
    if QMin['method'] == 0:
        dE = get_casscf_grad(mc,state)
    elif QMin['method'] == 3:
        dE = get_mcpdft_grad(mc,state)
    elif QMin['method'] == 4:
        dE = get_cms_grad(mc,state)
    
    else:
        print('Gradients: No PySCF support for given method!')
        sys.exit(12346)
    return dE

def get_dm(QMin,mc,state1,state2):
    if QMin['method'] in [0,4]:
        dm = get_casscf_dm(mc,state1,state2)
    elif QMin['method'] == 3:
        dm = get_mcpdft_dm(mc,state1,state2)
    else:
        print('No PySCF support for given method!')
        sys.exit(12347)
    return dm


def get_gssfile(QMin,job):
    if not QMin['template']['no-molcas-init']:
        if QMin['method']==0:
            #gssfile = os.path.join(QMin['scratchdir'],'master','MOLCAS.rasscf.h5')
            gssfile = os.path.join(QMin['scratchdir'],job,'MOLCAS.rasscf.h5')
        elif QMin['method'] in [3,4]:
            #gssfile = os.path.join(QMin['scratchdir'],'master','MOLCAS.rasscf.h5')
            gssfile = os.path.join(QMin['scratchdir'],job,'MOLCAS.rasscf.h5')
        else:
            print('PySCF only supports CASSCF and MC-PDFT so far!')
            sys.exit(1235)
    else:
        gssfile = None
    return gssfile
